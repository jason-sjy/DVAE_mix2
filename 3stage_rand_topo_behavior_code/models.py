...
class DVAE(nn.Module):
    def __init__(self, max_n, nvt, START_TYPE, END_TYPE, hs=501, nz=56, fs=16, bidirectional=False, vid=True):
            ...
            for i, g in enumerate(G):
                if finished[i]:
                    ...
                    end_vertices = set([v.index for v in g.vs.select(_outdegree_eq=0)
                                        if v.index != g.vcount() - 1 and v.index != g.vcount() - 2])
    ...
    def loss(self, mu, logvar, G_true, beta=0.005):
        ...
        for i in range(len(G_true)):
            if G_true[i].vcount() == 8:
                for j in range(G_true[i].vcount()):
                    print("graph ", i, " node ", j, " true param:", G_true[i].vs[j]['param'])
        """
        ...
        # feature_list = np.zeros(self.max_n, len(G_true), self.fs)
        # true_feature_list = np.zeros(self.max_n, len(G_true), self.fs)
        for v_true in range(1, self.max_n):
            ...
            # true_feature_list[v_true - 1] = true_feature
            ...
            for i, g in enumerate(G):
                if true_types[i] != self.START_TYPE:
                    ...
                    g.vs[v_true]['param'] = true_feature[i]
