...
# otherwise process the raw data and save to .pkl
    ...
    train_data, test_data, graph_args = load_circuit_graphs(topo_num, sizing_num)
...
def visualize_tsne(current_model):
    ...
    print(Z_tsne)
    d = np.where(id == 0)  # 找出聚类类别为0的数据对应的降维结果
    Z1 = Z[d]
    Y1 = Y[d]
    z1_mean = np.mean(Z1, axis=0)
    # plt.plot(Z_tsne[d, 0], Z_tsne[d, 1], 'g.', label = 'circuit 1')
    d = np.where(id == 1)  # 找出聚类类别为0的数据对应的降维结果
    Z2 = Z[d]
    Y2 = Y[d]
    z2_mean = np.mean(Z2, axis=0)
    # plt.plot(Z_tsne[d, 0], Z_tsne[d, 1], 'r*', label = 'circuit 2')
    # plt.legend(['circuit1', 'circuit2'])
    # plt.legend()
    # plt.legend(handles=[c1, c2], labels=['circuit1', 'circuit2'], loc='best')
    # plt.title('t-SNE Virtualization')
    # plt.show()
    print("z1_mean:", z1_mean)
    print("z2_mean:", z2_mean)
    print("Between-class distance is: ", np.linalg.norm(z1_mean - z2_mean))
    print("In-class distance of circuit 1 is: ", np.mean(np.linalg.norm(Z1 - z1_mean, axis=1)))
    print("In-class distance of circuit 2 is: ", np.mean(np.linalg.norm(Z2 - z2_mean, axis=1)))

    num = 100
    embedding_performance = np.zeros((num, 2))
    rand_id = np.random.randint(0, args.trainSet_size / 2, size=(num, 2))
    for i in range(num):
        # embedding_norm = np.linalg.norm(Z[rand_id[i][0]] - Z[rand_id[i][1]])
        # performance_norm = np.linalg.norm(Y[rand_id[i][0]] - Y[rand_id[i][1]])
        embedding_norm = np.dot(Z[rand_id[i][0]], Z[rand_id[i][1]]) / np.linalg.norm(Z[rand_id[i][0]]) / np.linalg.norm(
            Z[rand_id[i][1]])
        performance_norm = np.dot(Y[rand_id[i][0]], Y[rand_id[i][1]]) / np.linalg.norm(
            Y[rand_id[i][0]]) / np.linalg.norm(
            Y[rand_id[i][1]])
        # embedding_performance.append([embedding_norm, performance_norm])
        embedding_performance[i][0] = embedding_norm
        embedding_performance[i][1] = performance_norm
    plt.plot(embedding_performance[:, 0], embedding_performance[:, 1], 'bo')
    # plt.xlabel('embedding Euclidean distance')
    # plt.ylabel('performance Euclidean distance')
    plt.xlabel('embedding cosine distance')
    plt.ylabel('performance cosine distance')
    plt.show()

    num_z = 1000
    # mean = np.zeros(current_model.nz)
    mean = np.array([3.9480e-02, 1.6949e-01, 1.8252e-01, 6.8741e-02, -1.5694e-02,
                     -2.5756e-03, 1.0382e-01, -2.5616e-01, -2.5234e-01, 6.6900e-02])
    conv = np.eye(current_model.nz) / 1e6
    rand_z = np.random.multivariate_normal(mean=mean, cov=conv, size=num_z)
    rand_z = torch.from_numpy(rand_z).float()
    G = current_model.decode(rand_z)
    cnt = 0
    g_list = []
    new_graph_res_dir = os.path.join(args.res_dir, 'new_graph')
    if not os.path.exists(new_graph_res_dir):
        os.makedirs(new_graph_res_dir)
    for g in G:
        sign = 0
        for g1 in g_list:
            if is_same_DAG(g, g1):
        ...
        if sign == 0:
            g_list.append(g)
            name = 'new_graph_id{}'.format(cnt)
            plot_DAG(g, new_graph_res_dir, name)
            # print(args.res_dir)
            cnt = cnt + 1
    print('{} new topologies found'.format(cnt))
...
if args.only_test:
    '''Only testing'''
    # load_model_path  = os.path.join(args.file_dir, 'results_newDAG_e500_lr1e-5_nz30\\{}'.format(args.data_name))
    load_model_path = os.path.join(args.file_dir, 'results\\{}'.format(args.data_name))
    ...
    # print(os.path.join(load_model_path, load_model_name))
